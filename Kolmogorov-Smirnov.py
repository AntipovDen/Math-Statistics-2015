...
def ksDistance(distributions):
	f1, f2 = distributions
	s = max(abs(f1[i] - f2[i]) for i in range(len(f1)))
	return s

def l_emp(distributions):
	f1, f2 = distributions
	d = ksDistance(distributions) * sqrt(len(f1) * len(f2) / (len(f1) + len(f2)))
	return d 	

def hypothesis(distributions, alpha):
	c_a =  {
		0.1 : 1.22,
		0.05 : 1.36,
		0.025 : 1.48,
	...
	if l_emp(distributions) > c_a[alpha]:
		return False
	else:
		return True    
...
realDistance = ksDistance(([f1[1]] + [f1[i] - f1[i - 1] for i in range(1, n)], [f2[1]] + [f2[i] - f2[i - 1] for i in range(1, n)]))
print(x)
print(f)
...
def vectorResultKS(vector):
    return abs(ksDistance(getDistributions(x, vector)) - realDistance)

result = evolutionaryMinimizer(vectorResultKS, l)[1]
print(result)
print(f)

print("result:", ksDistance(getDistributions(x, result)),
      "real distribution:", realDistance,
      "difference:", vectorResultKS(result))

      
print ("for a = 0.01:", str(hypothesis(getDistributions(x, result), 0.01)))
print ("for a = 0.05:", str(hypothesis(getDistributions(x, result), 0.05)))
